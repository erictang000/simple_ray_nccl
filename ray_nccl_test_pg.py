...
if __name__ == "__main__":
    ...
    total_ranks = 8  # For example, 16 processes (1 per GPU across nodes)

    # Create a placement group with one GPU per actor.
    bundles = [{"GPU": 1, "CPU": 1} for _ in range(total_ranks)]
    pg_obj = placement_group(bundles, strategy="SPREAD")
    ray.get(pg_obj.ready())

    # Launch NCCLActor actors with proper placement.
    actors = []
    for rank in range(total_ranks):
        actor = NCCLActor.options(
            placement_group=pg_obj,
            placement_group_bundle_index=rank
        ).remote(rank, total_ranks)
        actors.append(actor)

    # Let rank 0 generate the NCCL unique id.
    unique_id = ray.get(actors[0].get_unique_id.remote())

    # Now run the NCCL broadcast test on all actors using the unique id.
